...
# --- 2. وظائف معالجة البيانات المتعددة ---
class MockMeta:
        ...
        with st.chat_message("assistant"):
            if df is None:
                ...
                with st.spinner('جاري البرمجة والتحليل بدقة...'):
                    ...
                    # التعديل هنا: إضافة تعليمة إجبارية لحساب المجموع في الجداول
                    prompt = f"""
                    ...
                    3. ➕ إجباري جداً: عند إنشاء أي جدول إحصائي (تكراري أو تقاطعي)، يجب إضافة صف وعمود لـ "المجموع" (Totals). استخدم خاصية `margins=True, margins_name='المجموع'` إذا كنت تستخدم `pd.crosstab` أو احسبها برمجياً.
                    4. إذا كان القاموس `value_labels` يحتوي على تراجم، استخدمه لترجمة الأرقام قبل الرسم:
                    ...
                    5. اكتب كود Python نظيف، وضع الكود بأكمله داخل علامات ```python و ```.
                    6. استخدم `st.dataframe()` للجدول.
                    7. استخدم `st.bar_chart()` للرسم، ولكن ⚠️ تأكد من إزالة صف وعمود "المجموع" قبل تمرير البيانات للرسم البياني حتى لا يتشوه المخطط.
